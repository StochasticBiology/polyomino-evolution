...
for(expt in 1:2) {
  if(expt == 1) {
    png("complexity-hists-directed.png", width=800, height=800)
    df = read.csv("stats-out-blend-1-0.100-10-16-16-64-1-0-1-5000-1e+08.csv", header=T)
  } else {
    png("complexity-hists-undirected.png", width=800, height=800)
  ...
  hist(df$CMNInterface[df$Size == 16], xlim=c(0,maxc), xlab="Complexity", main="All 16-mers")
  ...
  plot(new.df$Complexity[new.df$Time == t1], new.df$Count[new.df$Time == t1], xlab="Complexity", ylab="Frequency", main="With occurrence counts:\nAll structures\ndiscovered t<t1", type="b")
  plot(new.df$Complexity[new.df$Time == t2], new.df$Count[new.df$Time == t2], xlab="Complexity", ylab="Frequency", main="All structures\ndiscovered t<t2", type="b")
  plot(new.df$Complexity[new.df$Time == t3], new.df$Count[new.df$Time == t3], xlab="Complexity", ylab="Frequency", main="All structures\ndiscovered t<t3", type="b")
  plot(new.df$Complexity[new.df$Time == 20000], new.df$Count[new.df$Time == 20000], xlab="Complexity", ylab="Frequency", main="All structures", type="b")
  
  plot(new.16.df$Complexity[new.16.df$Time == t1], new.16.df$Count[new.16.df$Time == t1], xlab="Complexity", ylab="Frequency", main="16-mers\ndiscovered t<t1", type="b")
  plot(new.16.df$Complexity[new.16.df$Time == t2], new.16.df$Count[new.16.df$Time == t2], xlab="Complexity", ylab="Frequency", main="16-mers\ndiscovered t<t2", type="b")
  plot(new.16.df$Complexity[new.16.df$Time == t3], new.16.df$Count[new.16.df$Time == t3], xlab="Complexity", ylab="Frequency", main="16-mers\ndiscovered t<t3", type="b")
  plot(new.16.df$Complexity[new.16.df$Time == 20000], new.16.df$Count[new.16.df$Time == 20000], xlab="Complexity", ylab="Frequency", main="All 16-mers", type="b")
