...
png("plot-blend-3.png", width=800, height=800)
...
png("plot-blend-4.png", width=800, height=800)
...
plot.a = ggplot(sampling.df, aes(x = CMNInterface, y = log(SampleCount))) + geom_point()
plot.b = ggplot(undirected.df, aes(x = CMNInterface, y = log(DiscoveryCount))) + geom_point()
plot.c = ggplot(undirected.df, aes(x = log(SampleCount), y = log(DiscoveryCount))) + geom_point()
plot.d = ggplot(random.df, aes(x = CMNInterface, y = log(DiscoveryCount))) + geom_point()
plot.e = ggplot(undirected.df[undirected.df$Size == 16,], aes(x = CMNInterface, y = log(DiscoveryCount))) + geom_point()
plot.f = ggplot(directed.df, aes(x = log(SampleCount), y = log(DiscoveryCount))) + geom_point()

png("plot-blend-5.png", width=800, height=800)
...
png("plot-blend-6.png", width=800, height=800)
...
for(i in 1:nrow(sampling.df)) {
  mod.stats.df = rbind(mod.stats.df, data.frame(Mod = sampling.df$Modularity[i], Class = "Sampled", Complexity = sampling.df$CMNInterface[i], SymmGroup = symmgroups[sampling.df$Symmetry[i]+1]))
}
#for(i in 1:nrow(directed.df)) {
...
to.plot = subset(sampling.df, select=c(logP,CMNBlock,CMNInterface,CMNNonzero,CMLZW,CMNecklace,Size))
